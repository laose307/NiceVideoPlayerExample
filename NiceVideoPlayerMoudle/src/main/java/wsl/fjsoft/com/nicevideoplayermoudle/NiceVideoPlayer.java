...
public class NiceVideoPlayer extends FrameLayout
    ...
    private int imageShowTime = 3000;//如果没有视频，封面展现的时间长度
...
//                    ((IjkMediaPlayer)mMediaPlayer).setOption(4, "framedrop", 1L);
    ...
    private void openMediaPlayer() {
        ...
        try {
            //设置播放源头
            Log.d("NiceVideoPlayer", "mUrl:" + mUrl);

            String sdPath = FileDownUtils.getInstance().isHasVideo(mUrl);
            if (!TextUtils.isEmpty(sdPath)) {
                mUrl = sdPath;
            }
            Log.d("NiceVideoPlayer", "查找本地资源是否有文件:" + mUrl);
